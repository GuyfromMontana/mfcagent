...
// ============================================================================
// 11 LABS VOICE INTEGRATION FOR MONTANA FEED COMPANY API
// ============================================================================
...
// Process speech-to-text using Web Speech API results or external service
async function processSpeechToText(audioBuffer, options = {}) {
  try {
    // For now, we'll assume the client handles speech-to-text with Web Speech API
    // and sends us the transcribed text. For production, you might want to use:
...
// Generate speech response for Montana Feed Company
app.post('/api/voice/generate-speech', async (req, res) => {
  try {
...
app.post('/api/voice/conversation', async (req, res) => {
  try {
    ...
    const agentResponse = await generateMontanaFeedVoiceResponse(
      customer_message, 
      customer, 
      conversation
    );
...
// ============================================================================
// MONTANA FEED COMPANY VOICE RESPONSE LOGIC
// ============================================================================

async function generateMontanaFeedVoiceResponse(message, customer, conversation) {
  const lowerMessage = message.toLowerCase();
  let scoreIncrease = 5;
  let topics = [];
  ...
  // Advanced conversation analysis for voice
  let response = '';
...
What type of products are you looking at, and what's your typical feeding schedule? I can help set up regular deliveries so you never run short.`;
  ...
  else if (lowerMessage.includes('rangeland') || lowerMessage.includes('wind rain') || lowerMessage.includes('accuration')) {
...
// ============================================================================
// VOICE CONVERSATION END ENDPOINT
// ============================================================================
...
app.post('/api/voice/test', async (req, res) => {
  try {
    ...
    const testResponse = await generateMontanaFeedVoiceResponse(
      test_message, 
      null, // No customer context
      { lead_score: 0, transcript: [] }
    );
...
1. Add these environment variables to Railway:
   ELEVENLABS_API_KEY=sk_ca464e02d7ad7b9c603f97a89b2c044df8af45dbc082006f
   ELEVENLABS_VOICE_ID=6F0d2ON2P4jTXAWS1ohx

2. Add multer dependency to package.json:
   npm install multer

3. Add these routes and functions to your existing server.js file

4. Test the voice integration:
   POST https://your-railway-url/api/voice/test
